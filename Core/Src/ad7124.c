 ...
 * @brief   AD7124-4 24-bit Sigma-Delta ADC driver using STM32 HAL.
 *
 * @details
 * This driver enables communication with the AD7124-4 ADC through SPI transactions.
 * The current bring-up path polls the AD7124 status register rather than relying on EXTI.
 *
...
// Debug: number of samples seen per channel index
volatile uint32_t AD7124_ChSampleCount[8] = {0};
volatile uint8_t  AD7124_LastActiveChannel = 0xFF;
// Debug: history of last few active channels
uint8_t AD7124_DebugChLog[256];
uint16_t AD7124_DebugChIdx = 0;


// Debug: last low-level register read transaction.
// Watch these in CubeIDE if SPI/readback looks suspicious.
volatile uint8_t  AD7124_DebugLastReadReg      = 0U;
volatile uint8_t  AD7124_DebugLastReadSize     = 0U;
volatile uint8_t  AD7124_DebugLastReadTx[8U]   = {0U};
volatile uint8_t  AD7124_DebugLastReadRx[8U]   = {0U};
volatile uint8_t  AD7124_DebugLastReadLen      = 0U;
volatile uint8_t  AD7124_DebugLastCrcCalc      = 0U;
volatile uint8_t  AD7124_DebugLastCrcRx        = 0U;
volatile uint32_t AD7124_DebugLastReadValue    = 0U;
volatile int32_t  AD7124_DebugLastReadStatus   = 0;
volatile uint32_t AD7124_DebugLastSampleData   = 0U;
volatile uint8_t  AD7124_DebugLastStatusReg    = 0U;
...
AD7124_StatusTypeDef AD7124_ReadRegister(const AD7124_ConfigTypeDef *pADC, uint8_t regAddr, uint8_t dataSize, uint32_t *pRegValue)
{
    ...
    if ((pADC == NULL) || (pRegValue == NULL) || (dataSize == 0U) || (dataSize > 4U))
    {
        AD7124_DebugLastReadStatus = (int32_t)AD7124_ERROR;
        return AD7124_ERROR;
    }

    // Read command byte. The remaining bytes must be real dummy bytes, not
    // whatever happens to be adjacent to a single uint8_t on the stack.
    tx_buf[0] = (uint8_t)(0x40U | (regAddr & 0x3FU));

    // We are running with DATA_STATUS = 0 in ADC_CONTROL, so the DATA register
    // does not prepend a status byte. The final byte clocked back is the CRC.
    const uint8_t add_status_length = 0U;
    const uint8_t total_len = (uint8_t)(2U + dataSize + add_status_length);

    for (uint8_t i = 1U; i < total_len; i++)
    {
        tx_buf[i] = 0xFFU;
    }

    // Debug snapshot before the transaction.
    AD7124_DebugLastReadReg  = regAddr;
    AD7124_DebugLastReadSize = dataSize;
    AD7124_DebugLastReadLen  = total_len;
    for (uint8_t i = 0U; i < 8U; i++)
    {
        AD7124_DebugLastReadTx[i] = tx_buf[i];
        AD7124_DebugLastReadRx[i] = 0U;
    }

    HAL_GPIO_WritePin(pADC->csPort, pADC->csPin, GPIO_PIN_RESET);
    status = (AD7124_StatusTypeDef)HAL_SPI_TransmitReceive(
        pADC->SPIx,
        tx_buf,
        rx_buf,
        total_len,
    ...
    for (uint8_t i = 0U; i < total_len; i++)
    {
        AD7124_DebugLastReadRx[i] = rx_buf[i];
    }

    if (status != AD7124_OK)
    {
        AD7124_DebugLastReadStatus = (int32_t)status;
        return status;
    }

    // CRC is calculated over command + returned data bytes.
    uint8_t msg_length = 0U;
    msg_buf[msg_length++] = tx_buf[0U];
    for (uint8_t i = 0U; i < (uint8_t)(dataSize + add_status_length); i++)
    {
        msg_buf[msg_length++] = rx_buf[1U + i];
    }

    const uint8_t crc_calculated = AD7124_ComputeCRC8(msg_buf, msg_length);
    const uint8_t crc_received   = rx_buf[total_len - 1U];

    AD7124_DebugLastCrcCalc = crc_calculated;
    AD7124_DebugLastCrcRx   = crc_received;
    ...
    {
        data = (data << 8U) | rx_buf[1U + i];
    }

    *pRegValue = data;
    AD7124_DebugLastReadValue  = data;
    AD7124_DebugLastReadStatus = (int32_t)AD7124_OK;
...
//            }
//            AD7124_LastActiveChannel = active_channel;
//
//               if (active_channel < 8U)
//               {
//                   AD7124_ChSampleCount[active_channel]++;
//               }
//        }
//    }
//
...
AD7124_StatusTypeDef AD7124_ReadSampleData(const AD7124_ConfigTypeDef *pADC)
{
    ...
    // *** Read STATUS first ***
    status = AD7124_ReadRegister(pADC, AD7124_STATUS_REG, 1U, &status_reg);
    if (status != AD7124_OK)
    {
        return status;
    }
    ...
    active_channel = (uint8_t)(status_reg & 0x0FU);
    AD7124_LastActiveChannel = active_channel;
    AD7124_DebugLastStatusReg = (uint8_t)(status_reg & 0xFFU);

    // Then read the DATA for that conversion
    status = AD7124_ReadRegister(pADC, AD7124_DATA_REG, 3U, &sample_data);
    if (status != AD7124_OK)
    {
        return status;
    }

    AD7124_DebugLastSampleData = sample_data;
    ...
    // Debug ring buffer
    if (AD7124_DebugChIdx < 256)
    {
        AD7124_DebugChLog[AD7124_DebugChIdx++] = active_channel;
    }
...
//    uint8_t rx_buf[5] = {0};
//
//    // Command: 0x42 (Read Data Register)
//    // We send 5 bytes: [Command] [Dummy] [Dummy] [Dummy] [Dummy]
//    uint8_t tx_buf[5] = {0x42, 0xFF, 0xFF, 0xFF, 0xFF};
//
//    if (pADC == NULL) return AD7124_ERROR;
//
//    HAL_GPIO_WritePin(pADC->csPort, pADC->csPin, GPIO_PIN_RESET);
//    status = HAL_SPI_TransmitReceive(pADC->SPIx, tx_buf, rx_buf, 5, AD7124_MAX_DELAY);
//    HAL_GPIO_WritePin(pADC->csPort, pADC->csPin, GPIO_PIN_SET);
//
//    if (status == AD7124_OK) {
//        // Reassemble 24-bit Data
//        uint32_t data = ((uint32_t)rx_buf[1] << 16) | ((uint32_t)rx_buf[2] << 8) | rx_buf[3];
//
//        // Extract Channel ID from the 4th byte (Status)
//        // The Status byte is appended automatically because we set DATA_STATUS=1
//        uint8_t channel = rx_buf[4] & 0x0F;
//
//        // Save safely
//        AD7124_LastActiveChannel = channel;
//        if (channel < AD7124_ENABLED_CHANNELS) {
//            AD7124_ChannelSamples[channel] = data;
//        }
//    }
//    return status;
//}
